...
def test_api_product_metrics_search_endpoint():
    ...
    params2 = {
        ...
        "end_date": "2023-01-02",
        "product_id": 1,
        "census_division_name": "Wellington",
    }
...
def test_api_census_divisions_search_endpoint():
    url = DAIKON_HOST_URL + "/census-divisions/search"
    params = {"name": "Kawartha Lakes"}
    response_name_only = requests.get(url, params=params)

    data_name_only = response_name_only.json()

    expected_data_name_only = {
        "error": "Invalid query params, refer to https://tm41m.io/docs/daikon/census_divisions.html"
    }

    assert data_name_only == expected_data_name_only
